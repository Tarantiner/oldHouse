...
class Old58houseSpider(scrapy.Spider):
    ...
    def parse_urls(self, response):
        tmps = response.xpath('//ul[@class="house-list-wrap"]/li')
        for tmp in tmps:
            price = tmp.xpath('//div/p[@class="sum"]/text()').extract_first().strip('\n').strip()
            url = tmp.xpath('//h2[@class="title"]/a/@href').extract_first()
            if '?' in url:
                url = url.split('?')[0]
            if not url.startswith('https:'):
                url = 'https:' + url
            yield scrapy.Request(url, callback=self.parse_detail, meta={'price': price})
